...
// Helper types
  ...
  if (tool === "datadog") {
    ...
    const metricsByTech: Record<string, { label: string; m: string }[]> = {
      Kubernetes: [
        ...
        { label: "Disponibilidade", m: "kubernetes.pod.availability" },
      ],
      "Node.js": [
    ...
    const defaultMetrics = [
      ...
      { label: "Disponibilidade", m: "service.availability" },
    ];
 ...
 const CreateMonitor = () => {
  ...
  const generateImpactWithGemini = async (
    journeyVal: string,
    productVal: string
  ): Promise<string> => {
...
- Foque nos impactos ao negócio e na experiência do cliente, evitando jargões técnicos.`;
  ...
  const handleGenerateImpact = async () => {
    ...
    try {
      const result = await generateImpactWithGemini(journey.trim(), product.trim());
      if (!result) {
        setImpactError(
      ...
      if (e?.message === "SEM_CHAVE") {
        setImpactError(
          "Chave da Gemini não configurada no arquivo de configuração."
        );
      } else {
  ...
  const handleCreate = () => {
    ...
    if (tool === "datadog") {
      navigate("/monitores/datadog", { state: { serviceName, links: generatedLinks, monitorData } });
      return;
    }
