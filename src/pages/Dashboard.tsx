...
const Dashboard: React.FC = () => {
  const { monitors, clearMonitors } = useMonitorStorage();
  ...
  const getToolBadgeColor = (tool: string) => {
    switch (tool.toLowerCase()) {
      case 'datadog': return 'bg-purple-100 text-purple-800 border-purple-200 dark:bg-purple-900/20 dark:text-purple-300 dark:border-purple-800';
      case 'zabbix': return 'bg-blue-100 text-blue-800 border-blue-200 dark:bg-blue-900/20 dark:text-blue-300 dark:border-blue-800';
      case 'dynatrace': return 'bg-green-100 text-green-800 border-green-200 dark:bg-green-900/20 dark:text-green-300 dark:border-green-800';
  ...
  const filteredMonitors = useMemo(() => {
    return monitors.filter(monitor => {
      ...
      const matchesUser = selectedUser === "all" || 
        (monitor.user_email || 'Usuário da sessão').toLowerCase() === selectedUser.toLowerCase();
      
      const matchesSearch = searchTerm === "" || 
        monitor.service_name.toLowerCase().includes(searchTerm.toLowerCase()) ||
        monitor.tool.toLowerCase().includes(searchTerm.toLowerCase()) ||
        (monitor.user_email || 'Usuário da sessão').toLowerCase().includes(searchTerm.toLowerCase());
  ...
  const filteredStats = useMemo(() => {
    ...
    const avgMonitorTypes = total > 0 ? 
      filteredMonitors.reduce((sum, monitor) => sum + monitor.monitor_types.length, 0) / total : 0;
  ...
  const toolChartData = useMemo(() => 
    Object.entries(filteredStats.toolCounts).map(([name, value]) => ({
      name,
      value,
  ...
  const serviceChartData = useMemo(() => 
    Object.entries(filteredStats.serviceCounts)
  ...
  const userChartData = useMemo(() => 
    Object.entries(filteredStats.userCounts)
      .slice(0, 5) // Limitar a 5 principais
      .map(([name, value]) => ({
        name,
        value,
  ...
  const handleClearMonitors = () => {
    ...
    const confirmed = window.confirm(
      `Tem certeza que deseja limpar todos os ${monitors.length} monitores? Esta ação não pode ser desfeita.`
    );
      ...
      <div className="min-h-screen bg-background py-8">
        <main className="container mx-auto max-w-7xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6 mb-8">
                    ...
                    <div className="w-16 h-16 relative">
                      <svg className="w-full h-full transform -rotate-90" viewBox="0 0 100 100">
                        ...
                        <circle
                          ...
                          strokeWidth="8"
                          fill="none"
                          strokeDasharray={`${(filteredStats.total / generalStats.total) * 251.2} 251.2`}
                          strokeLinecap="round"
                      ...
                      <div className="absolute inset-0 flex items-center justify-center">
                        <span className="text-xs font-bold">
                          {Math.round((filteredStats.total / generalStats.total) * 100)}%
                        </span>
                      </div>
                    </div>
                  )}
            ...
            {/* Distribuição por Ferramentas */}
            <Card>
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <CardTitle className="text-sm font-medium">Por Ferramentas</CardTitle>
              ...
              <CardContent>
                {toolChartData.length > 0 ? (
                  <DonutChart
                    ...
                    centerLabel="Ferramentas"
                    centerValue={filteredStats.tools.length}
                    className="h-[180px]"
                  />
                ) : (
              ...
              <CardContent>
                {serviceChartData.length > 0 ? (
                  <DonutChart
                    ...
                    centerValue={filteredStats.services.length}
                    className="h-[180px]"
                  />
                ) : (
                  <div className="h-[180px] flex items-center justify-center text-muted-foreground">
              ...
              <CardContent>
                {userChartData.length > 0 ? (
                  <DonutChart
                    ...
                    centerValue={filteredStats.users.length}
                    className="h-[180px]"
                  />
                ) : (
                  <div className="h-[180px] flex items-center justify-center text-muted-foreground">
          ...
          <Card className="mb-6">
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4 mb-4">
                ...
                <div className="xl:col-span-2">
                  ...
                  <div className="relative">
                    ...
                    <Input
                      placeholder="Buscar por nome, ferramenta ou usuário..."
                      value={searchTerm}
              ...
              {Object.keys(filteredStats.toolCounts).length > 0 && (
                <div className="mt-4 flex flex-wrap items-center gap-2 text-sm text-muted-foreground">
                  <span>Distribuição filtrada por ferramenta:</span>
                  {Object.entries(filteredStats.toolCounts).map(([tool, count]) => (
                    <Badge key={tool} variant="outline" className={getToolBadgeColor(tool)}>
                      {tool}: {count}
                    </Badge>
                  ))}
                </div>
          ...
          {filteredMonitors.length === 0 ? (
            <Card>
              <CardContent className="text-center py-12">
                ...
                <p className="text-muted-foreground mb-4">
                  {monitors.length === 0 
                    ? "Comece criando seu primeiro monitor" 
            ...
            <div className="space-y-3">
              {filteredMonitors.map((monitor) => (
                <Card key={monitor.id} className="shadow-sm hover:shadow-md transition-all duration-200">
                  <CardContent className="p-6">
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <Badge 
                          variant="outline" 
                          className={`${getToolBadgeColor(monitor.tool)} text-sm px-3 py-1`}
                        >
                          {monitor.tool}
                        </Badge>
                      </div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="bg-muted/30 p-3 rounded-lg space-y-2 max-h-32 overflow-y-auto">
                          {monitor.monitor_links.length > 0 ? (
                            monitor.monitor_links.map((link, index) => (
                              <div 
                                key={index}
                    ...
                    {/* Tipos de monitores */}
                    {monitor.monitor_types.length > 0 && (
                      <div className="mt-6 pt-4 border-t border-border/50">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {monitor.monitor_types.map((type, index) => (
